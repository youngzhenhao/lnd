...
//
// 2) Varlen feature bit vector.
// 3) First blinding point: 33 bytes.
// 4) Number of hops: 1 byte.
// 5) Encoded BlindedHops.
func (p *BlindedPaymentPath) Encode(w io.Writer) error {
	...
	if numHops > math.MaxUint8 {
		return fmt.Errorf("the number of hops, %d, exceeds the "+
			"maximum of %d", numHops, math.MaxUint8)
	}
...
func EncodeBlindedHop(w io.Writer, hop *sphinx.BlindedHopInfo) error {
	...
	var dataLen [2]byte
	binary.BigEndian.PutUint16(dataLen[:], uint16(len(hop.CipherText)))
